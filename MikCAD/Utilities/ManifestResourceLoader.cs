...
namespace MikCAD
{
    ...
    public static class ManifestResourceLoader
    {
        ...
        public static string LoadTextFile(string directoryName, string textFileName)
        {
            var executingAssembly = Assembly.GetExecutingAssembly();
            var pathToDots = textFileName.Replace("\\", ".");
            var location = string.Format("{0}.{1}.{2}", executingAssembly.GetName().Name, directoryName,pathToDots);
